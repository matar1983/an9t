  ...
  export default function Levels() {
    ...
    return (
      <div className="max-w-5xl mx-auto space-y-6" dir="rtl">
        <div>
          <h1 className="text-3xl font-heading font-extrabold text-foreground mb-1">مستويات التعلّم</h1>
          <p className="text-muted-foreground">اختر مستوى لاستعراض دروسه المكتوبة، الاستماع للنطق، وإنهاء تقييمه.</p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-mono-en px-2.5 py-1 rounded-full bg-primary/15 text-primary">{lvl.cefr}</span>
                  {locked ? (
                ...
                <h3 className="text-lg font-heading font-bold text-foreground mb-2">{lvl.title}</h3>
                <p className="text-sm text-muted-foreground leading-relaxed mb-4">{lvl.description}</p>
                {p && (
                  <p className="text-xs text-muted-foreground mb-2">آخر نتيجة تقييم: {p.score}/{p.total} {passed ? "— ناجح" : "— أعد المحاولة"}</p>
                )}
                {!locked && (
                ...
                {locked && <p className="text-xs text-muted-foreground">أنهِ المستوى السابق أولاً لفتح هذا المستوى</p>}
              </button>
            );
          })}
        </div>
